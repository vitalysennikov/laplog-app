...
1. **Акценты для каждой активности** — главный вопрос. Например, для «дыхания
   медведя»: каждые сколько секунд звук? Какой? Есть ли звук на вдохе и отдельный
   на выдохе (через `startOffsetSeconds`)?

2. **Что делать, если пользователь удаляет активность из истории?**
   - Вариант А: при следующем запуске пресет не восстанавливается (флаг уже стоит).
   - Вариант Б: пресеты всегда есть — удалить нельзя (сложнее, не рекомендую).

3. **Нужны ли пресеты во втором бэкапе** (`laplog_backup_2026-05-21_140649.json`)?
   Там могут быть другие имена.

---

## Статус

- [ ] Заполнить акценты для всех 8 активностей
- [ ] Ответить на уточняющие вопросы
- [ ] Реализовать `ActivityPresets.kt`
- [ ] Добавить `seedActivityPresetsIfNeeded` в ViewModel
- [ ] Добавить флаг в PreferencesManager
